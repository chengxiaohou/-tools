...
- (void)setPhoto:(LGPhotoPickerBrowserPhoto *)photo{
    ...
    if (photo.photoPath.length) {
        ...
        if (photoRange.location != NSNotFound){
            ...
            #pragma mark 修改为AFN加载图片
            // 结果后来又放弃用AFN加载了，世道好轮回啊。。。。
            
//            // 此处改用AFN加载URL，为了保持图片缓存一致性
//            [_photoImageView setImageWithURLRequest:[NSURLRequest requestWithURL:photo.photoURL] placeholderImage:thumbImage success:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, UIImage * _Nonnull image) {
//                
...
//                    weakSelf.photoImageView.image = [UIImage imageNamed:@"icon_pic_break.png"];
//                    [weakSelf displayImage];
//                }
//                
//                
...
//                
//                NSLog(@"改用AFN不成功：%@", [error localizedDescription]);
//            }];
