...
{
    return [[KxMenuItem alloc] init:title image:image target:target clickEvent:handle];
}
...
- (UIView *) mkContentView
{
    ...
    for (KxMenuItem *menuItem in _menuItems) {
        ...
        const CGRect itemFrame = (CGRect){0, itemY-kMarginY * 2, maxItemWidth, maxItemHeight};
        ...
        if (menuItem.title.length) {
            ...
            titleLabel.textColor = [UIColor colorWithRed:self.kxMenuViewOptions.textColor.R/255.0 green:self.kxMenuViewOptions.textColor.G/255.0 blue:self.kxMenuViewOptions.textColor.B/255.0 alpha:1];
        ...
        if (itemNum < _menuItems.count - 1) {
            ...
            //配置：分隔线是与内容等宽还是与菜单等宽
            if (self.kxMenuViewOptions.seperatorLineHasInsets) {
                gradientView.frame = (CGRect){kMarginX * 2, maxItemHeight, gradientLine.size};
            } else {
...
//                itemY += 2;
    ...
    CGFloat newHeight = maxItemHeight * itemNum;
    
    contentView.frame = (CGRect){0, 0, maxItemWidth, newHeight};
    // 这里再次加个圆角，原来的方法不够实用，点击时圆角暴露了
    contentView.layer.cornerRadius = self.kxMenuViewOptions.menuCornerRadius;
    contentView.clipsToBounds = 1;
             ...
             inContext:(CGContextRef) context
{
    ...
    CGFloat R0 = self.kxMenuViewOptions.menuBackgroundColor.R /255.0, G0 = self.kxMenuViewOptions.menuBackgroundColor.G /255.0, B0 = self.kxMenuViewOptions.menuBackgroundColor.B /255.0;
