...
- (void)showNumberBadgeWithValue:(NSInteger)value
{
    ...
    self.badge.frame = frame;
    #warning 注意啦：我在这里对红点位置做了调整
    self.badge.center = CGPointMake(CGRectGetWidth(self.frame), self.badgeCenterOffset.y);
//    self.badge.center = CGPointMake(CGRectGetWidth(self.frame) + 2 + self.badgeCenterOffset.x, self.badgeCenterOffset.y);
...
- (void)badgeInit
{
    if (self.badgeBgColor == nil) {
        #warning 注意啦：颜色我改过
        self.badgeBgColor = HexColor(0xeb585e);
    }
...
#warning 注意啦：这里修改后cell点击时，红色背景色不会消失
...
- (void)setBadgeBgColor:(UIColor *)badgeBgColor
{
    ...
    if (self.badge) {
        ...
        self.badge.backgroundColor = [UIColor clearColor];
        self.badge.layer.backgroundColor = badgeBgColor.CGColor;
    }
}
...
#pragma mark 解决圆角不显示小红点
- (void)showCornerRadiusBadgeWithStyle:(WBadgeStyle)style value:(NSInteger)value animationType:(WBadgeAnimType)aniType
{
    [self.badge removeFromSuperview];
    self.badge = nil;
    
    [self showBadgeWithStyle:style value:value animationType:aniType];
    
    UILabel *lb = self.badge;
    CGRect rc = [self convertRect:lb.frame toView:self.superview];
    [self.superview addSubview:lb];
    lb.frame = rc;
    
    if (value == 0) {
        [self.badge clearBadge];
    }
}
